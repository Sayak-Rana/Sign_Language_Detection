...
def suggest_words(prefix, topk=5):
    """Suggest top-k completions for a prefix using BERT with beam search for word completion"""
    if not prefix:
        return []
    
    # Tokenize prefix
    init_tokens = tokenizer.tokenize(prefix)
    if not init_tokens:
        return []
    
    # Beam: list of (current_tokens, score)
    beam = [(init_tokens, 0.0)]
    completed = []
    max_add_tokens = 8  # Max additional tokens for completion
    
    for step in range(max_add_tokens):
        new_beam = []
        for curr_tokens, score in beam:
            # Prepare input with [MASK] at the end
            input_tokens = curr_tokens + [tokenizer.mask_token]
            inputs = tokenizer(input_tokens, is_split_into_words=True, return_tensors="pt").to(device)
            mask_indices = (inputs["input_ids"] == tokenizer.mask_token_id)[0].nonzero(as_tuple=True)[0]
            if len(mask_indices) == 0:
                continue
            mask_idx = mask_indices[-1]  # Last mask
            
            with torch.no_grad():
                logits = bert_model(**inputs).logits[0, mask_idx]
            
            probs = torch.softmax(logits, dim=-1)
            topk_ids = torch.topk(probs, 20).indices  # Take more to filter
            
            for tid in topk_ids:
                token = tokenizer.convert_ids_to_tokens(tid.item())
                new_score = score + math.log(probs[tid].item() + 1e-10)  # Log prob
                
                new_tokens = curr_tokens + [token]
                
                # If the new token is not a subword (doesn't start with ##) and we've added at least one token, consider it completed
                if not token.startswith('##') and len(new_tokens) > len(init_tokens):
                    word = tokenizer.convert_tokens_to_string(new_tokens)
                    if word.startswith(prefix) and all(c.isalpha() or c == "'" for c in word[len(prefix):]):  # Filter meaningful
                        completed.append((word, new_score))
                else:
                    new_beam.append((new_tokens, new_score))
        
        # Sort and keep top beams
        new_beam.sort(key=lambda x: x[1], reverse=True)
        beam = new_beam[:20]  # Keep more beams for diversity
        
        # If enough completed, break
        if len(completed) >= topk * 2:
            break
    ...
    unique_words = []
    seen = set()
    for word, _ in completed:
        if word not in seen:
            seen.add(word)
            unique_words.append(word)
        if len(unique_words) == topk:
            break
...
def predict_letter(frame, hands):
    ...
    # Apply transform without RGB conversion to match original asl_new.py
...
def main():
    ...
    while True:
        ...
        if prefix_changed:
            completions = suggest_words(current_prefix, topk=4)
            suggestions = [current_prefix] + completions
            if suggestions:
                print("Suggestions:")
